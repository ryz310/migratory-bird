...
const getEsaFormBodyTextForV1 = () => {
  const textArea = document.getElementById('post_body_md');
  return textArea && textArea.value;
}

const getEsaFormBodyTextForV2Beta = () => {
  const preTags = document.querySelectorAll('#js_tab_form .CodeMirror-lines pre');
  return toArray(preTags).reduce((memo, preTag) => memo += `${preTag.textContent}\n`, '');
}

const getEsaFormBodyText = () => {
  return getEsaFormBodyTextForV1() || getEsaFormBodyTextForV2Beta();
}
...
const getEsaFormBodyForV1 = () => {
  return document.querySelector('#post_form > div.form-group.form-body');
}

const getEsaFormBodyForV2Beta = () => {
  return document.querySelector('#js_tab_form .CodeMirror-lines');
}

const getEsaFormBody = () => {
  return getEsaFormBodyForV1() || getEsaFormBodyForV2Beta();
}
...
(() => {
  ...
  const esaFormBody = getEsaFormBody();
  if (esaFormBody !== null) { esaFormBody.appendChild(slackIconSvg) }
})();
